 ...
 */
const Milestone = ({ label, date, chip, caption, last }) => (
  <>
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 6, marginTop: 4 }}>
        <Text strong style={{ fontSize: 16 }}>{date ? formatDate(date) : '— not set —'}</Text>
        {chip}
      </div>
...
const DeadlinesPanel = ({ sr }) => {
  ...
  return (
    <Card size="small" title="Deadlines">
      <Milestone
      ...
      <Milestone
        ...
        chip={ragChip(sr.dispatchDeadline, shipped)}
        caption="Latest date to ship to the buyer"
      />
      <Milestone
        label="Buyer Approval"
        date={sr.buyerApprovalDeadline}
        chip={shipped ? ragChip(sr.buyerApprovalDeadline, false) : <Tag style={{ marginInlineEnd: 0 }}>Starts on dispatch</Tag>}
