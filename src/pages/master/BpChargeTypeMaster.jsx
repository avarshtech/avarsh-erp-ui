...
const BpChargeTypeMaster = ({ onDirtyChange }) => {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const list = await listChargeTypes();
      setData(list);
      setFilteredData(list);
    } catch (e) {
      message.error(e.message || 'Failed to load charge types');
    } finally {
      setLoading(false);
    }
  }, [message]);
  ...
  const columns = useMemo(() => [
    { title: 'Code', dataIndex: 'code', width: 160, render: (v) => <Tag color="blue">{v}</Tag> },
    { title: 'Name', dataIndex: 'name', sorter: (a, b) => (a.name || '').localeCompare(b.name || '') },
    { title: 'Taxable', dataIndex: 'defaultTaxable', align: 'center', width: 120, render: (v) => <Tag color={v ? 'geekblue' : 'default'}>{v ? 'Taxable' : 'Non-taxable'}</Tag> },
    { title: 'Status', dataIndex: 'active', align: 'center', width: 100, render: (v) => (v ? <Tag color="success">Active</Tag> : <Tag>Inactive</Tag>) },
  ], []);

  const handleAdd = useCallback(() => {
    if (!canAdd) { message.warning('You do not have permission to add charge types'); return; }
    skipDirty.current = true;
    setSelectedId(null);
    setIsEditing(true);
    form.resetFields();
    form.setFieldsValue({ defaultTaxable: true, active: true, sortOrder: (data.length + 1) * 10 });
    markDirty(false);
  ...
  const handleSave = useCallback(async (values) => {
    ...
    if (!code) { message.error('Charge type code is required'); return; }
    if (!name) { message.error('Charge type name is required'); return; }
    if (data.some((r) => (r.code || '').toUpperCase() === code && r.id !== selectedId)) {
      message.error('A charge type with this code already exists');
      return;
    }
    if (data.some((r) => (r.name || '').trim().toLowerCase() === name.toLowerCase() && r.id !== selectedId)) {
      message.error('A charge type with this name already exists');
      return;
    }

    setSubmitting(true);
    try {
      await saveChargeType({ ...values, code, name, id: selectedId || undefined });
      message.success(selectedId ? 'Charge type updated' : 'Charge type created');
      await loadData();
      handleCancel();
    } catch (e) {
      message.error(e.message || 'Failed to save charge type');
    } finally {
      setSubmitting(false);
    }
  ...
  const handleDelete = useCallback(() => {
    modal.confirm({
      ...
      content: 'This permanently removes the charge type. A type already used on a bill cannot be deleted — switch it to Inactive instead so past bills keep their history. Continue?',
      okText: 'Delete',
      okType: 'danger',
      cancelText: 'Cancel',
      onOk: async () => {
        try {
          await deleteChargeType(selectedId);
          message.success('Charge type deleted');
          await loadData();
          handleCancel();
        } catch (e) {
          message.error(e.message || 'Failed to delete charge type');
        }
      },
    });
      ...
      renderForm={() => (
        <div style={{ padding: 24 }}>
          <div className="master-form-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24, borderBottom: '1px solid var(--border-color)', paddingBottom: 16 }}>
            <h2 style={{ margin: 0 }}>{selectedId ? (isReadOnly ? 'View Charge Type' : 'Edit Charge Type') : 'New Charge Type'}</h2>
            <Space>
          ...
          >
            <Form.Item
              ...
              extra={selectedId ? 'Locked once saved — bills store this code against their charge lines.' : 'Uppercase, e.g. FREIGHT. Cannot be changed after saving.'}
              rules={[{ required: true, message: 'Please enter a code' }]}
            >
              <Input placeholder="e.g. FREIGHT" style={{ width: '60%' }} disabled={!!selectedId} />
            </Form.Item>
            <Form.Item name="name" label="Name" rules={[{ required: true, message: 'Please enter a name' }]}>
              <Input placeholder="e.g. Freight" size="large" />
            </Form.Item>
            <Form.Item name="defaultTaxable" label="Default Taxable" valuePropName="checked" extra="the charge joins the taxable value by default; it can still be overridden on a bill">
              <Switch />
            </Form.Item>
            <Form.Item name="sortOrder" label="Sort Order" extra="position of this type in the charge dropdown">
...
export default BpChargeTypeMaster;
