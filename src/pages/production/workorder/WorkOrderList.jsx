...
import { listWorkOrders } from '../../../services/production/productionService';
...
const WorkOrderList = () => {
        ...
        <Space size={0}>
          ...
          {EDITABLE_STATUSES.includes(r.status) && (
            <PermissionGuard module="production" operation="update">
              <ActionButton action="edit" onClick={() => navigate(`/production/work-order/edit/${r.id}`)} />
            </PermissionGuard>
          )}
        </Space>
      ) },
  ...
  return (
    <div className="animate-fade-in-up">
      <PageHeader title="Work Orders (Sewing)" backPath="/production">
        <PermissionGuard module="production" operation="add">
          <ActionButton action="create" text="New Work Order" onClick={() => navigate('/production/work-order/new')} />
        </PermissionGuard>
      </PageHeader>
