...
const TmbCheckGrid = ({ check, sizes, onChange }) => {
  ...
  const commentOptions = options('TMB_COMMENT');
  const actionOptions = options('TMB_ACTION');
  ...
  const columns = useMemo(() => [
    {
      ...
      render: (v, _, idx) => (
        <FormSelect size="small" value={v} style={{ width: 105 }} placeholder="Part"
          options={partOptions} onChange={(val) => setRow(idx, 'part', val)} />
      ),
    },
    {
    ...
    ...POSITIONS.map((pos) => ({
      ...
      render: (_, row, idx) => (
        <InputNumber size="small" step={0.1} controls={false} value={row[pos]} style={{ width: 80 }}
          onChange={(val) => setRow(idx, pos, val)} />
      ),
    })),
    {
    ...
    },
    {
      ...
      render: (v, _, idx) => (
        <FormSelect size="small" value={v} style={{ width: 125 }}
          options={commentOptions} onChange={(val) => setRow(idx, 'comment', val)} />
      ),
    },
    {
  ...
  ], [sizes, setRow, onChange, partOptions, commentOptions, actionOptions]);
  ...
  return (
    <Table
      ...
      pagination={false}
      scroll={{ x: 1000 }}
      rowClassName={(row) => (tmbRowInTolerance(row, tolerance) ? '' : 'row-shortage')}
